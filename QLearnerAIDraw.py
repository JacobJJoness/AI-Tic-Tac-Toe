...
class QLearningAI:
    def __init__(self, alpha=0.7, gamma=0.9, epsilon=0.5):
        self.q_table = {}
        self.alpha = alpha
        self.gamma = gamma
...
# Example of modifying the reward logic
def get_game_result(winner):
    if winner == "X":
        return 1  # Win for Q-learning AI (X)
    elif winner == "O":
        return -1  # Loss for Q-learning AI (X)
    else:
        return 0.1  # Small positive value for a draw


# Example usage in your game loop:
winner = "X"  # or "O" or "tie"
reward = get_game_result(winner)

# Update the Q-learning AI with the result
q_ai.update_q_table(state, action, reward, next_state, available_moves)
